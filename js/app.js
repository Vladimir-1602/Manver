 ...
 */
    ...
    (() => {
        ...
        const inputMask = document.querySelectorAll(".popup__input_mask");
        if (inputMask.length) flsModules.inputmask = Inputmask({
        ...
        function initSliders() {
            if (document.querySelector(".main-page__slider")) new core(".main-page__slider", {
                ...
                navigation: {
                    prevEl: ".main-page-prev",
                    nextEl: ".main-page-next"
                },
                on: {}
            });
            if (document.querySelector(".items-tabs__slider")) new core(".items-tabs__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 4,
                ...
                speed: 800,
                navigation: {
                    prevEl: ".items-tabs__prev",
                    nextEl: ".items-tabs__next"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 1,
            ...
            if (document.querySelector(".sliders-product__subslider")) var subSlider = new core(".sliders-product__subslider", {
                modules: [ Thumb ],
                observer: true,
                observeParents: true,
                slidesPerView: 7,
            ...
            if (document.querySelector(".sliders-product__mainslider")) new core(".sliders-product__mainslider", {
        ...
        document.addEventListener("click", (function(e) {
            let target = e.target;
            if (target.closest(".content-bottom__menu")) document.querySelector(".content-bottom__menu").classList.toggle("active");
            if (!target.closest(".content-bottom__menu")) document.querySelector(".content-bottom__menu").classList.remove("active");
            if (target.closest(".content-header__cart")) document.querySelector(".cart-list").classList.toggle("active");
            if (target.closest(".content-bottom__loop_mobile")) document.querySelector(".content-bottom__inp").classList.add("active");
            if (target.closest(".content-bottom__close") || !target.closest(".content-bottom__search")) document.querySelector(".content-bottom__inp").classList.remove("active");
            if (target.closest(".bottom-header__menumobile")) document.querySelector(".menu-header").classList.add("active");
            if (target.closest(".menu-header__close")) document.querySelector(".menu-header").classList.remove("active");
            if (target.closest(".top-header__item")) {
                e.preventDefault();
                target.closest(".top-header__item").classList.toggle("active");
            }
            if (target.closest(".nav-menu__button")) document.querySelector(".nav-menu__body").classList.add("active");
            if (target.closest(".filter-menu__button")) document.querySelector(".filter-menu__body").classList.add("active");
            if (target.closest(".close-menu")) {
                document.querySelector(".nav-menu__body").classList.remove("active");
                document.querySelector(".filter-menu__body").classList.remove("active");
            }
            ...
            if (target.closest(".cart-product__clean")) {
                let items = document.querySelectorAll(".cart-product__item");
                console.log(items);
                items.forEach((item => {
                    item.remove();
                }));
            ...
            if (target.closest(".information-cart__check")) document.querySelector(".order-cart").classList.add("active");
            if (target.closest(".header-order__back")) document.querySelector(".order-cart").classList.remove("active");
            if (target.closest(".favorites__mainclose")) target.parentElement.remove();
            if (target.closest(".favorites__close")) target.closest(".favorites__item").remove();
        }));
        const cartList = document.querySelector(".cart-list__body");
        const cartValue = document.querySelector(".content-header__value_cart");
        const fullPrice = document.querySelector(".cart-list__fullprice");
        const fullPriceZakaz = document.querySelector(".zakaz-cart__fullprice");
        const subFullPriceZakaz = document.querySelector(".zakaz-cart__price");
        const productButtons = document.querySelectorAll(".items-tabs__button");
        let startCartValue = 0;
        let price = 0;
        let randomId = () => Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
        let getValuesString = item => parseInt(item.replace(/[^\d]/g, ""));
        const normalPrice = str => String(str).replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, "$1 ");
        let plusPrice = currentPrice => price += currentPrice;
        productButtons.forEach((item => {
            item.setAttribute("data-id", randomId());
            item.addEventListener("click", (function(e) {
                let target = e.currentTarget;
                cartValue.style.display = "flex";
                cartValue.innerHTML = ++startCartValue;
                let itemParent = target.closest(".items-tabs__item");
                item.dataset.id;
                let img = itemParent.querySelector(".items-tabs__img img").getAttribute("src");
                let title = itemParent.querySelector(".items-tabs__title").textContent;
                let currentprice = parseInt(getValuesString(itemParent.querySelector(".items-tabs__currentprice").textContent));
                let oldprice = "";
                if (isFinite(parseInt(getValuesString(itemParent.querySelector(".items-tabs__oldprice").textContent)))) oldprice = parseInt(getValuesString(itemParent.querySelector(".items-tabs__oldprice").textContent));
                plusPrice(currentprice);
                fullPrice.innerHTML = normalPrice(price) + " ₽";
                cartList.insertAdjacentHTML("afterbegin", `\n      <div class="cart-list__item">\n      <div class="cart-list__img">\n         <img src="${img}" alt="">\n      </div>\n      <div class="cart-list__label">\n         <p>${title}</p>\n      </div>\n      <div class="cart-list__prices">\n         <div class="cart-list__oldprice">${oldprice + " ₽"}</div>\n         <div class="cart-list__currentprice">${normalPrice(currentprice)} ₽</div>\n      </div>\n   </div>\n</div> `);
            }));
        }));
        document.addEventListener("DOMContentLoaded", (function(e) {
            if (document.documentElement.classList.contains("touch")) {
                const categoryItems = document.querySelectorAll(".items-tabs__buttons");
        ...
        if (document.querySelector(".cart")) {
            let cartPrice = 0;
            var setFullSumm = () => {
                let cartFullPriceValue = 0;
                const cartPriceAllProducts = document.querySelectorAll(".item-cart__currentprice");
                cartPriceAllProducts.forEach((item => {
                    let itemValue = parseInt(getValuesString(item.textContent));
                    console.log(itemValue);
                    cartFullPriceValue += itemValue;
                }));
                cartFullPrice.innerHTML = normalPrice(cartFullPriceValue) + " ₽";
                fullPriceZakaz.innerHTML = normalPrice(cartFullPriceValue) + " ₽";
                subFullPriceZakaz.innerHTML = normalPrice(cartFullPriceValue) + " ₽";
                cartFullSubPrice.innerHTML = normalPrice(cartFullPriceValue) + " ₽";
            };
            const cartFullPrice = document.querySelector(".information-cart__info_fullprice");
            const cartFullSubPrice = document.querySelector(".information-cart__info_smallprice");
            const plusButtons = document.querySelectorAll(".item-cart__quantity .quantity__button_plus");
            const minusButtons = document.querySelectorAll(".item-cart__quantity .quantity__button_minus");
            var getAllItemsValue = () => {
                const allValueCartItems = document.querySelector(".cart-product__value");
                const allCartItemsLenght = document.querySelectorAll(".item-cart").length;
                allValueCartItems.innerHTML = allCartItemsLenght;
            };
            plusButtons.forEach((item => {
                const parent = item.closest(".item-cart__wrap");
                const value = parseInt(getValuesString(parent.querySelector(".item-cart__currentprice").textContent));
                console.log(value);
                item.addEventListener("click", (e => {
                    let target = e.currentTarget;
                    const parentCartElement = target.closest(".item-cart");
                    const cartProductElement = parentCartElement.querySelector(".item-cart__currentprice");
                    const productPrice = parseInt(getValuesString(parentCartElement.querySelector(".item-cart__currentprice").textContent));
                    cartPrice = value + productPrice;
                    cartProductElement.innerHTML = normalPrice(cartPrice) + " ₽";
                    setFullSumm();
                }));
            }));
            minusButtons.forEach((item => {
                ...
                item.addEventListener("click", (e => {
                    ...
                    const cartProductElement = parentCartElement.querySelector(".item-cart__currentprice");
                    const productPrice = parseInt(getValuesString(parentCartElement.querySelector(".item-cart__currentprice").textContent));
                    cartPrice = productPrice - value;
                    if (cartPrice < value) cartPrice = value;
                    cartProductElement.innerHTML = normalPrice(cartPrice) + " ₽";
                    setFullSumm();
                }));
            }));
